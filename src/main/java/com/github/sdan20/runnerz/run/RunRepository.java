...
public class RunRepository {
    ...
    public Optional<Run> findById(Integer id) {
        return jdbcClient.sql("SELECT id, title, started_one, completed_on, miles, location FROM RUN WHERE id = :id")
                .param("id", id)
                .query(Run.class)
                .optional();
    }
    ...
    public void update(Run run, Integer id) {
        var updated = jdbcClient.sql(
                "UPDATE RUN SET title = ?, started_one = ?, completed_on = ?, miles = ?, location = ? WHERE id = ?")
